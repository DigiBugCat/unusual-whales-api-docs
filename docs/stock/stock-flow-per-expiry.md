...
**Operation ID**: `PublicApi.TickerController.flow_per_expiry`
...
```bash
curl -X GET "https://api.unusualwhales.com/api/stock/{ticker}/flow-per-expiry" \
...
url = "https://api.unusualwhales.com/api/stock/{ticker}/flow-per-expiry"
headers = {
...
```javascript
const url = "https://api.unusualwhales.com/api/stock/{ticker}/flow-per-expiry";
const options = {
  method: 'GET',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY'
